...
<div class="woocommerce-shop-header">
	<div class="container">
		...
		<?php if ( is_shop() ) : ?>
			<h1><?php esc_html_e( 'GPS Tracking & Fleet Hardware Store', 'fleetlink' ); ?></h1>
			<p style="margin-top:.5rem; font-size:1rem; color:rgba(255,255,255,.75); max-width:500px;">
				<?php esc_html_e( 'Professional GPS trackers, OBD devices, dashcams and fleet management software subscriptions.', 'fleetlink' ); ?>
			</p>
		<?php else : ?>
...
<div class="shop-content">
	<div class="container">
		<div class="shop-layout">
			...
			<!-- Sidebar Filters -->
			<aside class="shop-sidebar" role="complementary" aria-label="<?php esc_attr_e( 'Shop filters', 'fleetlink' ); ?>">

				<!-- Search within shop -->
				<div class="filter-section">
					<h3 class="filter-title"><?php esc_html_e( 'Search Products', 'fleetlink' ); ?></h3>
					<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
						<div style="position:relative;">
							<input type="search" name="s" placeholder="<?php esc_attr_e( 'Search…', 'fleetlink' ); ?>"
							...
							<button type="submit" style="position:absolute; right:.5rem; top:50%; transform:translateY(-50%); background:none; border:none; cursor:pointer; color:var(--color-gray-500);">
								<?php echo fleetlink_icon( 'search' ); // phpcs:ignore ?>
							</button>
						</div>
					</form>
				</div>
				...
				<!-- Product Categories -->
				<div class="filter-section">
					<h3 class="filter-title"><?php esc_html_e( 'Categories', 'fleetlink' ); ?></h3>
					<ul style="display:flex; flex-direction:column; gap:.25rem;">
						...
						if ( ! empty( $product_cats ) && ! is_wp_error( $product_cats ) ) {
							...
							$defaults = array(
								__( 'GPS Trackers', 'fleetlink' ),
								__( 'OBD Devices', 'fleetlink' ),
								__( 'Dashcams', 'fleetlink' ),
								__( 'Fleet Software', 'fleetlink' ),
								__( 'Accessories', 'fleetlink' ),
							);
				...
				<!-- Price Range -->
				<div class="filter-section">
					<h3 class="filter-title"><?php esc_html_e( 'Price Range', 'fleetlink' ); ?></h3>
					<?php
					...
					if ( is_active_sidebar( 'sidebar-shop' ) ) {
						...
						<form method="get" action="<?php echo esc_url( wc_get_shop_url() ); ?>">
							...
							<button type="submit" class="btn btn-primary btn-sm" style="margin-top:.75rem; width:100%;">
								<?php esc_html_e( 'Apply Filter', 'fleetlink' ); ?>
							</button>
						</form>
					<?php } ?>
				</div>
				...
				<!-- Product Tags -->
				<div class="filter-section">
					<h3 class="filter-title"><?php esc_html_e( 'Tags', 'fleetlink' ); ?></h3>
				...
				<?php if ( woocommerce_product_loop() ) : ?>
					...
					<div class="shop-toolbar">
						...
						<div class="shop-toolbar-right">
							...
							<div class="view-toggles" role="group" aria-label="<?php esc_attr_e( 'View', 'fleetlink' ); ?>">
								<button class="view-toggle-btn active" data-view="grid" aria-label="<?php esc_attr_e( 'Grid view', 'fleetlink' ); ?>" aria-pressed="true">
									<?php echo fleetlink_icon( 'grid' ); // phpcs:ignore ?>
								</button>
								<button class="view-toggle-btn" data-view="list" aria-label="<?php esc_attr_e( 'List view', 'fleetlink' ); ?>" aria-pressed="false">
									<?php echo fleetlink_icon( 'list' ); // phpcs:ignore ?>
								</button>
							</div>
						</div>
					</div>
