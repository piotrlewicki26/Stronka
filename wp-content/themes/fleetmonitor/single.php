...
<div class="page-hero">
	<div class="container">
		<nav class="breadcrumb-list" aria-label="<?php esc_attr_e( 'Breadcrumb', 'fleetmonitor' ); ?>">
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Home', 'fleetmonitor' ); ?></a>
			&rsaquo;
			<a href="<?php echo esc_url( home_url( '/blog/' ) ); ?>"><?php esc_html_e( 'Blog', 'fleetmonitor' ); ?></a>
			&rsaquo;
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'entry-article' ); ?>>
				<?php if ( has_post_thumbnail() ) : ?>
					<div style="margin-bottom: 2rem; border-radius: var(--radius-xl); overflow: hidden;">
						<?php the_post_thumbnail( 'fleetmonitor-card', array( 'style' => 'width:100%; height:auto;' ) ); ?>
					</div>
				<?php endif; ?>
				...
				<div class="entry-content">
					...
					the_content(
						sprintf(
							wp_kses(
								__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'fleetmonitor' ),
				...
				<?php $tags = get_the_tags(); if ( $tags ) : ?>
					<div class="entry-tags" style="margin-top:2rem; padding-top:1.5rem; border-top:1px solid var(--color-gray-200);">
						<strong style="font-size:.875rem; color:var(--color-gray-600);"><?php esc_html_e( 'Tags:', 'fleetmonitor' ); ?></strong>
				...
				<!-- Navigation -->
				<nav class="post-navigation" style="margin-top:2rem; display:grid; grid-template-columns:1fr 1fr; gap:1rem;" aria-label="<?php esc_attr_e( 'Post navigation', 'fleetmonitor' ); ?>">
					<?php
					...
					if ( $prev ) : ?>
						<a href="<?php echo esc_url( get_permalink( $prev ) ); ?>" style="display:block; padding:1rem; background:var(--color-white); border:1px solid var(--color-gray-200); border-radius:var(--radius-lg); text-decoration:none;">
							<span style="font-size:.75rem; color:var(--color-gray-500); text-transform:uppercase; letter-spacing:.05em;">&larr; <?php esc_html_e( 'Previous', 'fleetmonitor' ); ?></span>
					...
					<?php if ( $next ) : ?>
						<a href="<?php echo esc_url( get_permalink( $next ) ); ?>" style="display:block; padding:1rem; background:var(--color-white); border:1px solid var(--color-gray-200); border-radius:var(--radius-lg); text-decoration:none; text-align:right; grid-column: <?php echo $prev ? 'auto' : '2'; ?>;">
							<span style="font-size:.75rem; color:var(--color-gray-500); text-transform:uppercase; letter-spacing:.05em;"><?php esc_html_e( 'Next', 'fleetmonitor' ); ?> &rarr;</span>
