...
<div class="single-product-hero">
	<div class="container">
		<!-- Breadcrumb -->
		<nav style="font-size:.875rem; color:rgba(0,0,0,.5); margin-bottom:2rem;" aria-label="<?php esc_attr_e( 'Breadcrumb', 'fleetmonitor' ); ?>">
		...
		<div class="single-product-layout" id="product-<?php the_ID(); ?>" <?php wc_product_class( '', $product ); ?>>
			...
			<div class="product-gallery-area">
				<div class="product-gallery-main" id="product-gallery-main">
					...
					if ( $image_id ) {
						echo wp_get_attachment_image( $image_id, 'fleetmonitor-square', false, array( 'id' => 'product-main-image' ) );
					} else {
					...
					<div class="product-gallery-thumbs">
						...
						if ( $image_id ) {
							printf(
								...
								esc_attr( $image_id ),
								esc_attr__( 'Main product image', 'fleetmonitor' ),
						...
						foreach ( $gallery_image_ids as $gid ) {
							printf(
								...
								esc_attr( $gid ),
								esc_attr( get_post_meta( $gid, '_wp_attachment_image_alt', true ) ) ?: esc_attr__( 'Product gallery image', 'fleetmonitor' ),
			...
			<div class="product-summary-area">
				...
				<div style="margin-top:1.5rem; padding:1.5rem; background:var(--color-gray-100); border-radius:var(--radius-xl); display:grid; grid-template-columns:repeat(3,1fr); gap:1rem;">
					<?php
					$trust = array(
						array( 'icon' => '🚚', 'text' => __( 'Free shipping over €200', 'fleetmonitor' ) ),
						array( 'icon' => '🔒', 'text' => __( '2-year warranty', 'fleetmonitor' ) ),
						array( 'icon' => '↩️', 'text' => __( '30-day returns', 'fleetmonitor' ) ),
					);
					foreach ( $trust as $t ) :
					?>
...
<section style="background:var(--color-gray-100); padding:4rem 0;">
	<div class="container">
		<h2 style="font-size:1.75rem; font-weight:800; color:var(--color-primary); margin-bottom:2rem;">
			<?php esc_html_e( 'Related Products', 'fleetmonitor' ); ?>
		</h2>
